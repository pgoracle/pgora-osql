...
/*--------------------------------------------------------------------------------------------
 *
 * gram.y			- Parser for the PL/OSQL procedural language
 *
...
%type <stmt>	plosql_only_stmt plpgsql_only_stmt common_stmt
...
/*
 * Other tokens recognized by plosql's lexer interface layer (pl_scanner.c).
 */
...
pl_function		: comp_options pl_block opt_semi
					{
						plosql_parse_result = (PLTSQL_stmt_block *) $2;
					}
				;
...
pl_block		: decl_sect K_BEGIN proc_sect exception_sect K_END
					{
						...
						new->lineno		= plosql_location_to_lineno(@2);
						new->label		= $1.label;
				...
				| opt_block_label decl_start decl_stmts
					{
						...
						$$.n_initvars = plosql_add_initdatums(&($$.initvarnos));
					}
				;
...
decl_statement	: decl_varname decl_const decl_datatype decl_collate decl_notnull decl_defval
					{
						...
						var = plosql_build_variable($1.name, $1.lineno,
													 $3, true);
						if ($2)
						{
				...
				| decl_varname K_ALIAS K_FOR decl_aliasitem opt_semi_or_commma
					{
						plosql_ns_additem($4->itemtype,
										   $4->itemno, $1.name);
					}
				| decl_varname opt_scrollable K_CURSOR
					{ plosql_ns_push($1.name); }
				  decl_cursor_args decl_is_for decl_cursor_query
					{
						...
						new = (PLTSQL_var *)
							plosql_build_variable($1.name, $1.lineno,
				...
				| '(' decl_cursor_arglist ')'
					{
						...
						new->lineno = plosql_location_to_lineno(@1);
						new->rowtupdesc = NULL;
...
decl_varname	: T_WORD
					{
						$$.name = $1.ident;
						$$.lineno = plosql_location_to_lineno(@1);
						/*
...
for_control		: for_variable K_IN
					{
								 ...
								 ((PLTSQL_var *) yylval.wdatum.datum)->datatype->typoid == REFCURSOROID)
						{
							...
							new->rec = plosql_build_record($1.name,
															$1.lineno,
															true);
...
/*
 * T_WORD+T_CWORD match any initial identifier that is not a known plosql
 ...
 * word is a SQL statement keyword that isn't also a plosql keyword.
...
stmt_fetch		: K_FETCH opt_fetch_direction cursor_variable K_INTO
					{
						...
						fetch->lineno = plosql_location_to_lineno(@1);
						fetch->rec		= rec;
						fetch->row		= row;
						fetch->curvar	= $3->dno;
						fetch->is_move	= false;
				...
				| K_EXCEPTION
					{
						 ...
						 * current block.
						 */
						int			lineno = plosql_location_to_lineno(@1);
...
opt_label	:
				...
				| any_identifier
					{
						if (plosql_ns_lookup_label(plosql_ns_top(), $1) == NULL)
