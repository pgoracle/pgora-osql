...
IdentifierLookup plosql_IdentifierLookup = IDENTIFIER_LOOKUP_NORMAL;
 ...
 * isn't a known variable name.  If plosql_IdentifierLookup is DECLARE then
 ...
 * into a YY_EXTRA struct.	For the moment, there is no need for plosql's
...
/* Current token's length (corresponds to plosql_yylval and plosql_yylloc) */
static int	plosql_yyleng;
...
/* State for plosql_location_to_lineno() */
 ...
 * is turned off by plosql_IdentifierLookup, it is returned as
...
plosql_yylex(void)
{
		...
		if (tok2 == '.')
		{
			...
			if (tok3 == IDENT)
			{
				...
				if (tok4 == '.')
				{
					...
					if (tok5 == IDENT)
					{
						if (plosql_parse_tripword(aux1.lval.str,
												   aux3.lval.str,
												   aux5.lval.str,
												   &aux1.lval.wdatum,
												   &aux1.lval.cword))
					...
					else
					{
						...
						if (plosql_parse_dblword(aux1.lval.str,
												  aux3.lval.str,
												  &aux1.lval.wdatum,
												  &aux1.lval.cword))
							tok1 = T_DATUM;
			...
			else
			{
				...
				if (plosql_parse_word(aux1.lval.str,
	...
	else
	{
		...
		if (tok1 == Op)
		{
			...
			else if (aux1.leng == 1 && aux1.lval.str[0] == '@')
			{
				...
				if (tok2 == IDENT)
				{
					...
					if (plosql_parse_word(var_word.data,
					                      var_word.data,
					                      &aux1.lval.wdatum,
					                      &aux1.lval.word))
						tok1 = T_DATUM;
		...
		/* Not a potential plosql variable name, just return the data */
	}

	plosql_yylval = aux1.lval;
	plosql_yylloc = aux1.lloc;
	plosql_yyleng = aux1.leng;
	return tok1;
}
...
/*
 * Push back a single token to be re-read by next plosql_yylex() call.
 *
 ...
 */
void
plosql_push_back_token(int token)
{
	TokenAuxData auxdata;

	auxdata.lval = plosql_yylval;
	auxdata.lloc = plosql_yylloc;
	auxdata.leng = plosql_yyleng;
 ...
 */
void
plosql_append_source_text(StringInfo buf,
 ...
 */
void
plosql_peek2(int *tok1_p, int *tok2_p, int *tok1_loc, int *tok2_loc)
{
	int			tok1,
				tok2;
	TokenAuxData aux1,
 ...
 * parsing of a plosql function, since it requires the scanorig string
 * to still be available.
 */
int
plosql_scanner_errposition(int location)
{
	int			pos;
 ...
 */
void
plosql_yyerror(const char *message)
{
	char	   *yytext = core_yy.scanbuf + plosql_yylloc;
	...
	if (*yytext == '\0')
	{
		...
		/* translator: %s is typically the translation of "syntax error" */
				 ...
				 plosql_scanner_errposition(plosql_yylloc)));
	}
	else
	{
		/*
 ...
 */
int
plosql_location_to_lineno(int location)
{
	const char *loc;
...
/* initialize or reset the state for plosql_location_to_lineno */
static void
 ...
 *
 * Note: the passed "str" must remain valid until plosql_scanner_finish().
 * Although it is not fed directly to flex, we need the original string
 * to cite in error messages.
 */
void
plosql_scanner_init(const char *str)
{
...
/*
 * Called after parsing is done to clean up after plosql_scanner_init()
 */
void
plosql_scanner_finish(void)
{
	/* release storage */
