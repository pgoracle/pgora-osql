/*--------------------------------------------------------------------------------------------
 *
 * pl_funcs.c		- Misc functions for the PL/OSQL
 *			  procedural language
 *
 ...
 * any one plosql statement.  During initial parsing of a function, ns_top
...
/* ----------
 * plosql_ns_init			Initialize namespace processing for a new function
 * ----------
 */
void
plosql_ns_init(void)
...
/* ----------
 * plosql_ns_push			Create a new namespace level
 * ----------
 */
void
plosql_ns_push(const char *label)
{
	if (label == NULL)
		label = "";
	plosql_ns_additem(PLTSQL_NSTYPE_LABEL, 0, label);
}


/* ----------
 * plosql_ns_pop			Pop entries back to (and including) the last label
 * ----------
 */
void
plosql_ns_pop(void)
...
/* ----------
 * plosql_ns_top			Fetch the current namespace chain end
 * ----------
 */
PLTSQL_nsitem *
plosql_ns_top(void)
...
/* ----------
 * plosql_ns_additem		Add an item to the current namespace chain
 * ----------
 */
void
plosql_ns_additem(int itemtype, int itemno, const char *name)
{
	PLTSQL_nsitem *nse;
...
/* ----------
 * plosql_ns_lookup		Lookup an identifier in the given namespace chain
 *
 ...
 */
PLTSQL_nsitem *
plosql_ns_lookup(PLTSQL_nsitem *ns_cur, bool localmode,
...
/* ----------
 * plosql_ns_lookup_label		Lookup a label in the given namespace chain
 * ----------
 */
PLTSQL_nsitem *
plosql_ns_lookup_label(PLTSQL_nsitem *ns_cur, const char *name)
{
	while (ns_cur != NULL)
	{
 ...
 */
const char *
plosql_stmt_typename(PLTSQL_stmt *stmt)
{
 ...
 */
const char *
plosql_getdiag_kindname(int kind)
{
	switch (kind)
	{
...
void
plosql_free_function_memory(PLTSQL_function *func)
{
	int			i;
...
dump_getdiag(PLTSQL_stmt_getdiag *stmt)
{
	...
	foreach(lc, stmt->diag_items)
	{
		...
		printf("{var %d} = %s", diag_item->target,
			   plosql_getdiag_kindname(diag_item->kind));
	}
	printf("\n");
}
...
void
plosql_dumptree(PLTSQL_function *func)
{
	int			i;
	PLTSQL_datum *d;
