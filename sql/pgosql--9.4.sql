...
/* PL/OSQL procedural language */

CREATE FUNCTION plosql_call_handler ()
RETURNS language_handler AS 'pgtsql' LANGUAGE C;

CREATE FUNCTION plosql_validator (oid)
RETURNS void AS 'pgtsql' LANGUAGE C;

-- language
CREATE LANGUAGE plosql 
       HANDLER plosql_call_handler 
       VALIDATOR plosql_validator;

COMMENT ON LANGUAGE plosql IS 'PL/SQL procedural language';
